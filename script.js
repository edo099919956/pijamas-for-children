...
document.querySelector(".add").addEventListener("click", () => {
    ...
    if (
        ...
        function elements_count(dv) {
            let all_dv = document.querySelectorAll(".basket_open_close>div")
            for (i = 0; i < all_dv.length + 1; i++) {
                document.querySelector(".basket_min_count").innerHTML = all_dv.length
            }
        }
        elements_count()

        function count__see() {
            if (document.querySelectorAll(".basket_open_close>div").length === 0) {
                document.querySelector(".basket_min_count").style.opacity = "0";
            } else {
                document.querySelector(".basket_min_count").style.opacity = "1";
            }
        }
        count__see()
        ...
        btn2_close.addEventListener("click", () => {
            ...
            somenewArr.forEach((val, index) => {
                ...
                if (val.id == index) {
                    if (somenewArr.length > 1) {
                        somenewArr.splice(index + 1, 1);
